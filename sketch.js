...
function setup() {
  ...
  blobs.push(new Blob(500, w/2, h/2));
  blobs.push(new Blob(150,w/2+200,h/2));
...
function draw() {
  ...
  strokeWeight(17.5);
  //blob 내부 색 바꾸기 - fill(red,green,blue,alpha);
  //fill(255,255,255,100);
  fill(mouseX%256, mouseY%256, mouseX%256, mouseY%256);
...
function Blob(r,coX,coY){
  ...
  this.attractor = new Particle(new Vec2D(coX, coY), r, r, 0.001);
  this.attractor.lock();
  ...
  for (var i = 0; i < 50; i++) {
    //스프링 정의하기
    var spring1 = new VerletSpring2D(this.particles[i], this.particles[(i + 1) % this.particles.length], 2*Math.PI, 0.5);
